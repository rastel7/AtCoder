...
int main() {
  ...
  REP(i, N)cin >> w[i] >> v[i];
  map<ll, ll> mp1, mp2;
  map<ll, ll>* now = &mp1, * next = &mp2;
  if(W < w[0]) {
    cout << 0 << endl;
    return 0;
  }
  now->insert(P(w[0], v[0]));
  for(int i = 1;i < N;++i) {
    next->clear();
    for(auto& x : *now) {
      if(x.first + w[i] <= W) {
        if(next->count(x.first + w[i]) != 0) {
          auto nex = next->at(x.first + w[i]);
          if(nex < x.second + v[i]) {
            next->erase(x.first + w[i]);
            next->insert(P(x.first + w[i], x.second + v[i]));
          }
        } else {
          next->insert(P(x.first + w[i], x.second + v[i]));
        }
      }
      if(next->count(x.first) != 0) {
        auto nex = next->at(x.first);
        if(nex < x.second) {
          next->erase(x.first);
          next->insert(P(x.first, x.second));
        }
      } else {
        next->insert(P(x.first, x.second));
      }
    ...
    if(next->count(w[i]) != 0) {
      auto nex = next->at(w[i]);
      if(nex < v[i]) {
        next->erase(w[i]);
        next->insert(P(w[i], v[i]));
      }
    } else {
      next->insert(P(w[i], v[i]));
    }
