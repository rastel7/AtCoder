...
int main() {
  ...
  cin >> A >> B >> C;
  double tl, tr;
  double tmp = 1 / (2 * C), tmp2 = 3 / (2 * C);
  if(A * tmp >= 100) {
    tl = 0, tr = tmp;
  } else {
    int l = 0;
    while(true) {
      tl = l * (2 / C) + tmp, tr = tl + 2 * tmp;
      if(f(tl) <= 100 && 100 <= f(tr)) {
        break;
      }
      tl = tr, tr = tl + 2 * tmp;
      if(f(tl) <= 100 && 100 <= f(tr)) {
  ...
  cerr << tl << " : " << tr << endl;
  cerr << fixed << setprecision(7) << f(tl) << " : " << f(tr) << endl;
  while(abs(f(tl) - f(tr)) >= 1e-8) {
    double mid1 = tl + (tr - tl) / 3, mid2 = tl + (tr - tl) / 3 * 2;
    if(f2(mid1) > f2(mid2)) {
      tl = mid1;
    } else {
      tr = mid2;
  ...
  cout << fixed << setprecision(10) << (tl + tr) / 2.0 << endl;
  cerr << f((tl + tr) / 2.0) << endl;
  cerr << f(0.3761368912) << endl;
}