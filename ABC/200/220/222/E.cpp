...
int edge_num[(int)1e5];
struct edge {
  int to, id;
};
bool dfs(int now, int prev, vector<vector<edge>> const& G, int goal) {
  if(now == goal) {
    return true;
  }
  for(auto& g : G[now]) {
...
int main() {
  ...
  vector<int> nums;
  REP(i, N) {
    if(edge_num[i] != 0)nums.emplace_back(edge_num[i]);
  }
  map<int, mint> dp;
  dp[0] = 1;
  for(auto& x : nums) {
    ...
    for(auto& y : dp) {
      ...
      next[y.first + x] += y.second;
    }
    dp = next;
  }
  int sum_edge = 0;
  int pw = 0;
  for(auto& x : nums)sum_edge += x;
  REP(i, N - 1)if(edge_num[i] == 0)pw++;
  mint ans = 0;
  for(auto& x : dp) {
    auto f = [&](int R, int B) {
      if(R - B == K)ans += x.second;
    };
    f(x.first, sum_edge - x.first);
    //f(sum_edge - x.first, x.first);
  }
  cout << (ans * mint(2).pow(pw)).val() << endl;
}