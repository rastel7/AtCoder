...
int main() {
  int N, M;
  cin >> N >> M;
  vector<P> score(2 * N);//勝ち、番号
  REP(i, 2 * N)score[i] = P(0, i);
  vector<vector<char>> A(2 * N, vector<char>(M));
  REP(i, 2 * N)REP(j, M)cin >> A[i][j];
  REP(i, M) {
    sort(score.begin(), score.end(), [](P l, P r) { if(l.first != r.first) { return l.first > r.first; } else { return l.second < r.second; }});
    REP(j, N) {
      int x = score[2 * j].second, y = score[2 * j + 1].second;
      char l = A[x][i], r = A[y][i];
      if(l == r)continue;
      auto f = [&](char c, char k) {
        if(l == c) {
          if(r == k) {
            score[2*j].first++;
          } else {
