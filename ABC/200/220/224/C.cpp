  ...
  for(int i = 0;i < N;++i) {
    for(int j = i + 1;j < N;++j) {
      for(int k = j + 1;k < N;++k) {
        ...
        vector<P> v = { P(X[i],Y[i]),P(X[j],Y[j]),P(X[k],Y[k]) };
        sort(v.begin(), v.end());
        pair<ll, ll> v1 = make_pair(v[0].first - v[1].first, v[0].second - v[1].second);
        pair<ll, ll> v2 = make_pair(v[0].first - v[2].first, v[0].second - v[2].second);
        if(v1.first * v2.second == v1.second * v2.first)continue;
        ans++;
      }
    }
  }
