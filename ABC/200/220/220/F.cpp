...
int dfs(int id, int prev, vector<vector<int>> const& G, vector<int>& childs) {
  if(childs[id] != -1)return childs[id];
  int ret = 1;
  for(auto& e : G[id]) {
    if(e == prev)continue;
    ret += dfs(e, id, G, childs);
  }
  childs[id] = ret;
  return ret;
}
void ans_dfs(int id, int prev, vector<vector<int>> const& G, vector<int> const& childs, vector<ll>& ans) {
  ll tmp = ans[prev];
  tmp -= childs[id];
  tmp += (G.size() - childs[id]);
  ans[id] = tmp;
  for(auto& x : G[id]) {
    if(x == prev)continue;
    ans_dfs(x, id, G, childs, ans);
  }
}
int main() {
  int N;
  ...
  dfs(0, -1, G, childs);
  vector<ll> dis;
  vector<vector<lib::graph::Edge>> g(N);
  for(int i = 0;i < N;++i) {
    for(auto& x : G[i]) {
      g[i].emplace_back(lib::graph::Edge{ x,1 });
    }
  }
  lib::graph::dijkstra(g, 0, dis);
  ll sum_0 = 0;
  REP(i, N)sum_0 += dis[i];
  vector<ll> ans(N);
  ans[0] = sum_0;
  for(auto& x : G[0]) {
    ans_dfs(x, 0, G, childs, ans);
  }
  REP(i, N) {
    cout << ans[i];
    cout << endl;
