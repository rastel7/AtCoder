...
struct t {
  ...
  bool operator<(const t& right) const {
    ...
    for(int i = 0;i < min(s.size(), right.s.size());++i) {
      if(s[i] == right.s[i])continue;
      return se[s[i]] < se[right.s[i]];
    }
    return s.size() < right.s.size();
  };
};
int main() {
  string T;
  cin >> T;
  REP(i, T.size())se[T[i]] = i;
  int N;
  cin >> N;
  vector<t> S(N);
  REP(i, N)cin >> S[i].s;
  sort(S.begin(), S.end());
  REP(i, N)cout << S[i].s << endl;
}