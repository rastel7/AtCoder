...
int main() {
  ...
  REP(i, N)cin >> S[i];
  REP(i, N)cin >> T[i];
  int x_0 = INF, y_0 = INF, x_1 = -1, y_1 = -1;
  REP(i, N)REP(j, N) {
    if(S[i][j] == '#') {
      x_0 = min(x_0, j);
      y_0 = min(y_0, i);
  ...
  REP(i, N) {
    if(y_0 <= i && i <= y_1) {
      S_n.emplace_back(S[i].substr(x_0, (x_1 - x_0 + 1)));
    }
  }
  for(auto s : S_n) {
    //cerr << s << endl;
  }
  auto func = [](vector<string> S, vector<string> T) {
    int N = T.size();
    REP(x, N)REP(y, N) {
      if(x == 3 && y == 2) {
        int afdaddjas = 243243;
      }
      if(x + S[0].size() > N || y + S.size() > N)continue;
      bool ok = true;
      REP(i, S.size())REP(j, S[0].size()) {
        if(S[i][j] != T[i + y][j + x])ok = false;
      }
      if(ok)return true;
    }
    return false;
  };
  auto rorate_ = [](vector<string> S) {
    vector<string> ret(S[0].size());
    REP(i, S.size())REP(j, S[i].size()) {
      ret[j] += S[i][S[i].size() - 1 - j];
    }
    cerr << endl;
    REP(i, ret.size()) {
      cerr << ret[i] << endl;
