...
int main() {
  int N, M;
  cin >> N >> M;
  vector<vector<lib::graph::Edge>> G(N),fromN(N);
  vector<pair<P, int>> edges(M);
  REP(i, M) {
    int s, t;
    cin >> s >> t, s--, t--;
    edges[i] = make_pair(P(s, t), G[s].size());
    G[s].emplace_back(lib::graph::Edge{ t,1 });
    fromN[t].emplace_back(lib::graph::Edge{ s,1 });
  }
  vector<ll> dis_0, dis_N;
  dijkstra(G, 0, dis_0);
  dijkstra(fromN, N - 1, dis_N);
  if(dis_0[N - 1] >= INFll) {
    REP(i, M)cout << -1 << endl;
    return 0;
  }
  REP(i, M) {
    ...
    if(dis_0[p.first] + dis_N[p.second] + 1 != dis_0[N - 1]) {
      cout << dis_0[N - 1] << endl;
      continue;
    }
    G[p.first][edges[i].second] = Edge{ p.second,INF };
    vector<ll> dis;
    dijkstra(G, 0, dis);
    if(dis[N - 1] >= INF)dis[N - 1] = -1;
    cout << dis[N - 1] << endl;
    G[p.first][edges[i].second] = Edge{ p.second,1 };
  }
}