...
int main() {
  ...
  vector<string> S(H);
  REP(i, H)cin >> S[i];
  vector<vector<P>> poss(26);
  REP(i, H)REP(j, W) {
    char c = S[i][j];
    if('a' <= c && c <= 'z') {
      poss[c - 'a'].emplace_back(P(i, j));
    }
  }
  P s, g;
  REP(i, H)REP(j, W) {
  ...
  while(!que.empty()) {
    ...
    que.pop();
    REP(i, 4)REP(j, 4) {
      int y = dy[i] + id.first, x = dx[i] + id.second;
      if(OoR(y, x))continue;
      if(S[y][x] == '#')continue;
      if(dis[y][x] <= dis[id.first][id.second] + 1)continue;
      dis[y][x] = dis[id.first][id.second] + 1;
      que.push(P(y, x));
    }
    char c = S[id.first][id.second];
    if(!('a' <= c && c <= 'z')) {
      continue;
    }
    if(touched[c - 'a'] == false) {
      touched[c - 'a'] = 1;
      for(auto& p : poss[c - 'a']) {
        if(p == id)continue;
        dis[p.first][p.second] = min(dis[p.first][p.second], dis[id.first][id.second] + 1);
        que.push(p);
      }
    }
  }
  if(dis[g.first][g.second] >= INF)dis[g.first][g.second] = -1;
